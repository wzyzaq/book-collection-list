...
var createClass = require('../../_chunks/dep-e711efe7.js');
var defineProperty = require('../../_chunks/dep-5481698f.js');
var utils = require('../../_chunks/dep-68f49619.js');
  ...
  return createClass._createClass(TableTreeStore, [{
    ...
    value: function getAllUniqueKeys(data, keys) {
      ...
      for (var i = 0, len = data.length; i < len; i++) {
        var item = data[i];
        arr.push(get.get(item, keys.rowKey));
    ...
    value: function toggleExpandData(p, dataSource, keys) {
      if (!p) {
        log.log.error("EnhancedTable", "the node you want to toggleExpand doest not exist in `data`");
        return dataSource;
      }
    ...
    value: function updateData(rowValue, newRowData, dataSource, keys) {
      var newRowValue = get.get(newRowData, keys.rowKey);
      ...
      if (get.get(newRowData, keys.childrenKey) === true) {
        ...
        if (oldChildren !== null && oldChildren !== void 0 && oldChildren.length) {
          for (var i = 0, len = oldChildren.length; i < len; i++) {
            var rowValue2 = get.get(oldChildren[i], keys.rowKey);
      ...
      if (rowState.parent) {
        ...
        var index = siblings.findIndex(function (item) {
          return get.get(item, keys.rowKey) === rowValue;
        });
    ...
    value: function appendTo(rowValue, newData, dataSource, keys) {
      ...
      for (var i = 0, len = tmpData.length; i < len; i++) {
        ...
        var newRowValue = get.get(oneData, keys.rowKey);
